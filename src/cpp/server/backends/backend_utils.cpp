...
namespace lemon::backends {
    ...
    std::vector<std::string> BackendUtils::get_therock_lib_paths(const std::string& rocm_arch) {
        ...
        // runtime is split across several directories (recorded in runtime_paths.txt
        // at install time); ALL are needed on the loader path so a ROCm consumer can
        // resolve both HIP DLLs (_rocm_sdk_core/bin) and BLAS DLLs
        // (_rocm_sdk_libraries/bin).
        {
            fs::path paths_file =
